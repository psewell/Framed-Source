		...
		if (rightArm ~= nil) then
			...
			if (joint ~= nil and (joint.Part0 == Handle or joint.Part1 == Handle)) then
				...
				if player and not HitPlayers[player] then
					...
					local theirTorso = player.Character:FindFirstChild'Torso'
					if math.abs((math.deg(math.acos(myTorso.CFrame.lookVector:Dot(theirTorso.CFrame.lookVector))))) > 90 then
						game.Workspace.Events.Shoot:FireServer(nil, player.Character.Humanoid, script.Parent, 30)
					else
