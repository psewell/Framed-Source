function refresh()
	...
	for _, shirt in pairs(game.ReplicatedStorage.Clothes.Shirts:GetChildren()) do
		local bought = false
		for _, test in pairs(inventory['shirts']) do
		...
		if not bought then
			...
			item.Category.Value = "shirts"
			item.ItemName.Text = shirt.Name
			item.Preview.Image = "rbxassetid://" .. shirt.Preview.Value
